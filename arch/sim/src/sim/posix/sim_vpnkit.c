...
static int vpnkit_connect(void)
{
  ...
  fd = host_uninterruptible_errno(socket, AF_UNIX, SOCK_STREAM, 0);
  if (fd < 0)
    {
  ...
  if (host_uninterruptible_errno(connect, fd, (const struct sockaddr *)&sun,
                                 sizeof(sun)) < 0)
    {
      if (!g_connect_warned)
        {
  ...
  if (host_uninterruptible(negotiate, fd, &g_vifinfo))
    {
  ...
  sim_netdriver_setmacaddr(0, g_vifinfo.mac);
  return 0;
}
...
int sim_vpnkit_avail(void)
{
  ...
  if (ret == -1)
    ...
    {
      DEBUG("sim_vpnkit_avail is returning 1");
    }
