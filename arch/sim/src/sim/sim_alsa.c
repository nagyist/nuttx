...
static int sim_audio_config_format(struct sim_audio_s *priv, snd_pcm_t *pcm)
{
  ...
  ret = host_snd_pcm_hw_params_malloc(&hw_params);
  if (ret < 0)
    {
      return ret;
    }

  ret = host_snd_pcm_hw_params_any(pcm, hw_params);
  if (ret < 0)
    {
      goto fail;
    }

  ret = host_snd_pcm_hw_params_set_access(pcm, hw_params,
  ...
  ret = host_snd_pcm_hw_params_set_format(pcm, hw_params, format);
  if (ret < 0)
    {
      goto fail;
    }
  ...
  ret =
      host_snd_pcm_hw_params_set_rate(pcm, hw_params, priv->sample_rate, 0);
  if (ret < 0)
    {
      goto fail;
    }

  ret = host_snd_pcm_hw_params_set_channels(pcm, hw_params, priv->channels);
  if (ret < 0)
    {
      goto fail;
    }
  ...
  ret = host_snd_pcm_hw_params_set_period_size_near(pcm, hw_params, &pframes,
                                                    NULL);
  if (ret < 0)
    {
      goto fail;
  ...
  ret = host_snd_pcm_hw_params_set_periods_near(pcm, hw_params,
                                                &priv->nbuffers, NULL);
  if (ret < 0)
    {
      goto fail;
  ...
  ret = host_snd_pcm_hw_params(pcm, hw_params);

fail:
  host_snd_pcm_hw_params_free(hw_params);
  return ret;
}
...
static int sim_audio_open(struct sim_audio_s *priv)
{
  ...
  ret = host_snd_pcm_open(&pcm, "default", direction, 0);
  if (ret < 0)
    {
      return ret;
    }
        ...
        switch (caps->ac_format.hw)
          {
            case AUDIO_FU_VOLUME:
              host_snd_mixer_selem_get_playback_volume(
                                   priv->volume,
          ...
          host_snd_mixer_selem_get_capture_volume(priv->volume,
  ...
  switch (caps->ac_type)
    {
      case AUDIO_TYPE_FEATURE:
        switch (caps->ac_format.hw)
          {
            case AUDIO_FU_VOLUME:
              ret = host_snd_mixer_selem_set_playback_volume_all(
                      priv->volume, caps->ac_controls.hw[0]);
              break;
            case AUDIO_FU_INP_GAIN:
              ret = host_snd_mixer_selem_set_capture_volume_all(
                        priv->volume, caps->ac_controls.hw[0]);
              break;
            default:
              ret = -ENOTTY;
                                ...
                                unsigned long arg)
{
  ...
  ret = host_snd_pcm_delay(priv->pcm, latency);
  if (ret < 0)
    {
      return ret;
    }
                                      ...
                                      bool *dequeue)
{
  ...
  if (ret < 0)
  ...
  else
    {
      ...
      ret = host_snd_pcm_writei(priv->pcm, out, outsize / priv->frame_size);
      if (ret > 0)
        ret *= priv->frame_size;
    }
                                     ...
                                     int expect, bool *dequeue)
{
  ...
  frames = host_snd_pcm_readi(priv->pcm, apb->samp, expect);
  if (frames < 0)
    {
      return frames;
    }
...
static void sim_audio_process(struct sim_audio_s *priv)
{
  ...
  avail = host_snd_pcm_avail(priv->pcm);
  ...
  if (ret == -EPIPE)
    {
      ...
      host_snd_pcm_prepare(priv->pcm);
      host_snd_pcm_start(priv->pcm);
    }
  else if (ret != -EAGAIN)
    {
      aerr("pcm writei/readi failed %d, %s\n", ret,
           host_snd_strerror(ret));
    }
}
...
static int sim_mixer_open(struct sim_audio_s *priv)
{
  ...
  ret = host_snd_mixer_open(&priv->mixer, 0);
  if (ret < 0)
    {
      goto fail;
    }

  ret = host_snd_mixer_attach(priv->mixer, "default");
  if (ret < 0)
    {
      goto fail;
    }

  ret = host_snd_mixer_selem_register(priv->mixer, NULL, NULL);
  if (ret < 0)
    {
      goto fail;
    }

  ret = host_snd_mixer_load(priv->mixer);
  if (ret < 0)
    {
      goto fail;
    }

  ret = host_snd_mixer_selem_id_malloc(&sid);
  if (ret < 0)
    {
      goto fail;
    }
  ...
  if (priv->playback)
    {
      host_snd_mixer_selem_id_set_index(sid, 0);
      host_snd_mixer_selem_id_set_name(sid, "Master");

      priv->volume = host_snd_mixer_find_selem(priv->mixer, sid);
      host_snd_mixer_selem_id_free(sid);
      if (!priv->volume)
        {
          ret = -EINVAL;
          goto fail;
      ...
      ret = host_snd_mixer_selem_set_playback_volume_range(priv->volume, 0,
                                                           1000);
      if (ret < 0)
        {
          goto fail;
    ...
    {
      host_snd_mixer_selem_id_set_index(sid, 0);
      host_snd_mixer_selem_id_set_name(sid, "Capture");
      ...
      ret = host_snd_mixer_selem_set_capture_volume_range(priv->volume, 0,
                                                          1000);
      if (ret < 0)
        {
          goto fail;
  ...
  return 0;
fail:
  host_snd_mixer_close(priv->mixer);
  priv->mixer = NULL;
  return ret;
}
