...
static void esp32c3twai_reset(struct can_dev_s *dev);
static int  esp32c3twai_setup(struct can_dev_s *dev);
static void esp32c3twai_shutdown(struct can_dev_s *dev);
static void esp32c3twai_rxint(struct can_dev_s *dev, bool enable);
static void esp32c3twai_txint(struct can_dev_s *dev, bool enable);
static int  esp32c3twai_ioctl(struct can_dev_s *dev, int cmd,
                              unsigned long arg);
static int  esp32c3twai_remoterequest(struct can_dev_s *dev,
                                      uint16_t id);
static int  esp32c3twai_send(struct can_dev_s *dev,
                             struct can_msg_s *msg);
static bool esp32c3twai_txready(struct can_dev_s *dev);
static bool esp32c3twai_txempty(struct can_dev_s *dev);

/* TWAI interrupts */

static int esp32c3twai_interrupt(int irq, void *context, void *arg);
...
static int esp32c3twai_remoterequest(struct can_dev_s *dev, uint16_t id)
{
...
struct can_dev_s *esp32c3_twaiinitialize(int port)
{
