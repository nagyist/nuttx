...
static const struct wlan_ops_s g_softap_ops =
{
...
static int esp32c3_net_initialize(int devno, uint8_t *mac_addr,
...
{
  struct wlan_priv_s *priv = &g_wlan_priv[ESP32C3_WLAN_SOFTAP_DEVNO];
...
int esp32c3_wlan_sta_initialize(void)
{
  ...
  ret = esp32c3_net_initialize(ESP32C3_WLAN_STA_DEVNO, eth_mac, &g_sta_ops);
  if (ret < 0)
    {
...
int esp32c3_wlan_softap_initialize(void)
{
  ...
  ret = esp32c3_net_initialize(ESP32C3_WLAN_SOFTAP_DEVNO, eth_mac,
                             &g_softap_ops);
  if (ret < 0)
    {
      nerr("ERROR: Failed to initialize net\n");
