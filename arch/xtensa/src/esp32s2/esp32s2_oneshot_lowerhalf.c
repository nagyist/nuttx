...
struct esp32s2_oneshot_lowerhalf_s
{
  /* This is the part of the lower half driver that is visible to the upper-
   * half client of the driver.  This must be the first thing in this
   * struct so that pointers to struct oneshot_lowerhalf_s are cast
   * compatible to struct esp32s2_oneshot_lowerhalf_s and vice versa.
   */
...
static void esp32s2_oneshot_lh_handler(void *arg);
...
static const struct oneshot_operations_s g_esp32s2_timer_ops =
{
...
/****************************************************************************
 * Name: esp32s2_oneshot_lh_handler
 *
 * Description:
 *   Timer expiration handler.
 *
 * Input Parameters:
 *   arg - Should be the same argument provided when esp32s2_oneshot_start()
 *         was called.
 *
 ****************************************************************************/
                                ...
                                struct timespec *ts)
{
   ...
   * 1 us.
   * Therefore, here, set the timespec with the
...
{
  struct esp32s2_oneshot_lowerhalf_s *priv =
    (struct esp32s2_oneshot_lowerhalf_s *)lower;
  int ret;
  irqstate_t flags;
  ...
  if (ret < 0)
    {
      tmrerr("ERROR: esp32s2_oneshot_start failed: %d\n", ret);
    }
                             ...
                             struct timespec *ts)
{
  ...
  if (ret < 0)
    {
      tmrerr("ERROR: esp32s2_oneshot_cancel failed: %d\n", flags);
    }
                              ...
                              struct timespec *ts)
{
  ...
  esp32s2_oneshot_current(&priv->oneshot, &current_us);
                                               ...
                                               uint16_t resolution)
{
  ...
  ret = esp32s2_oneshot_initialize(&priv->oneshot, chan, resolution);
  if (ret < 0)
    {
      tmrerr("ERROR: esp32s2_oneshot_initialize failed: %d\n", ret);
      kmm_free(priv);
      return NULL;
    }
