                         ...
                         uintptr_t offset)
{
  ...
  DEBUGASSERT(irq >= STM32L5_IRQ_NMI && irq < NR_IRQS);

  /* Check for external interrupt */

  if (irq >= STM32L5_IRQ_FIRST)
    {
      n        = irq - STM32L5_IRQ_FIRST;
  ...
  /* Handle processor exceptions.  Only a few can be disabled */
  ...
  else
    {
      *regaddr = NVIC_SYSHCON;
      if (irq == STM32L5_IRQ_MEMFAULT)
        {
          *bit = NVIC_SYSHCON_MEMFAULTENA;
        }
      else if (irq == STM32L5_IRQ_BUSFAULT)
        {
          *bit = NVIC_SYSHCON_BUSFAULTENA;
        }
      else if (irq == STM32L5_IRQ_USAGEFAULT)
        {
          *bit = NVIC_SYSHCON_USGFAULTENA;
        }
      else if (irq == STM32L5_IRQ_SYSTICK)
        {
...
void up_irqinitialize(void)
{
  ...
  for (i = 0; i < NR_IRQS - STM32L5_IRQ_FIRST; i += 32)
    {
  ...
  irq_attach(STM32L5_IRQ_SVCALL, arm_svcall, NULL);
  irq_attach(STM32L5_IRQ_HARDFAULT, arm_hardfault, NULL);
  ...
  up_enable_irq(STM32L5_IRQ_MEMFAULT);
#endif

  /* Attach all other processor exceptions (except reset and sys tick) */

#ifdef CONFIG_DEBUG_FEATURES
  irq_attach(STM32L5_IRQ_NMI, stm32l5_nmi, NULL);
...
int up_prioritize_irq(int irq, int priority)
{
  ...
  DEBUGASSERT(irq >= STM32L5_IRQ_MEMFAULT && irq < NR_IRQS &&
