/****************************************************************************
 * arch/arm/src/stm32f7/stm32_i2s.c
 *
...
static uint32_t i2s_mckdivider(struct stm32_i2s_s *priv)
{
  ...
  i2s_dump_regs(priv, "After i2s_mckdivider");
  ...
  if (priv->rxenab)
    {
      ...
      priv->rx.dma = stm32_dmachannel(DMACHAN_I2S2_RX);
      if (!priv->rx.dma)
        {
  ...
  if (priv->txenab)
    {
      ...
      priv->tx.dma = stm32_dmachannel(DMACHAN_I2S2_TX);
      if (!priv->tx.dma)
        {
  ...
  if (!priv->initialized)
    {
      ...
      stm32_configgpio(GPIO_I2S1_MCK);
      stm32_configgpio(GPIO_I2S1_SD);
      stm32_configgpio(GPIO_I2S1_CK);
