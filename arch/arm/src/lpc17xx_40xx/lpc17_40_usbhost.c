...
#  define CONFIG_LPC17_40_USBHOST_NPREALLOC 8
#endif
...
#  define LPC17_40_DEBUG_INTS      (OHCI_INT_SO|OHCI_INT_RD|OHCI_INT_UE|OHCI_INT_OC)
#else
#  define LPC17_40_DEBUG_INTS      0
#endif

#define LPC17_40_NORMAL_INTS       (OHCI_INT_WDH|OHCI_INT_RHSC)
#define LPC17_40_ALL_INTS          (LPC17_40_NORMAL_INTS|LPC17_40_DEBUG_INTS)
...
#  define usbhost_dumpgpio() \
   do { \
     ...
     lpc17_40_dumpgpio(GPIO_USB_UPLED, "LED P1:18; PPWR P1:19 PWRD P1:22 PVRCR P1:27"); \
   } while (0);
#else
#  define usbhost_dumpgpio()
#endif
...
#define HCCA        ((struct ohci_hcca_s *)LPC17_40_HCCA_BASE)
#define TDTAIL      ((struct lpc17_40_gtd_s *)LPC17_40_TDTAIL_ADDR)
#define EDCTRL      ((struct lpc17_40_ed_s *)LPC17_40_EDCTRL_ADDR)
...
struct lpc17_40_usbhost_s
{
  /* Common device fields.  This must be the first thing defined in the
   * structure so that it is possible to simply cast from struct usbhost_s
   * to structlpc17_40_usbhost_s.
   */
...
struct lpc17_40_gtd_s
{
  ...
  struct lpc17_40_ed_s *ed;      /* Pointer to parent ED */
  uint8_t           pad[12];
};
...
static void lpc17_40_tdfree(struct lpc17_40_gtd_s *buffer);
static uint8_t *lpc17_40_tballoc(void);
static void lpc17_40_tbfree(uint8_t *buffer);
#if LPC17_40_IOBUFFERS > 0
static uint8_t *lpc17_40_allocio(void);
static void lpc17_40_freeio(uint8_t *buffer);
#endif
...
static struct lpc17_40_list_s *g_edfree; /* List of unused EDs */
static struct lpc17_40_list_s *g_tdfree; /* List of unused TDs */
static struct lpc17_40_list_s *g_tbfree; /* List of unused transfer buffers */
#if LPC17_40_IOBUFFERS > 0
static struct lpc17_40_list_s *g_iofree; /* List of unused I/O buffers */
#endif
...
static inline void lpc17_40_edfree(struct lpc17_40_ed_s *ed)
{
  struct lpc17_40_list_s *entry = (struct lpc17_40_list_s *)ed;
...
static struct lpc17_40_gtd_s *lpc17_40_tdalloc(void)
{
  ...
  ret   = (struct lpc17_40_gtd_s *)g_tdfree;
  if (ret)
    {
      g_tdfree = ((struct lpc17_40_list_s *)ret)->flink;
    }
...
static void lpc17_40_tdfree(struct lpc17_40_gtd_s *td)
{
  struct lpc17_40_list_s *tdfree = (struct lpc17_40_list_s *)td;
...
static void lpc17_40_tbfree(uint8_t *buffer)
{
  struct lpc17_40_list_s *tbfree = (struct lpc17_40_list_s *)buffer;
...
static uint8_t *lpc17_40_allocio(void)
{
  ...
  /* lpc17_40_freeio() may be called from the interrupt level */

  flags = enter_critical_section();
  ret = (uint8_t *)g_iofree;
  if (ret)
    {
      g_iofree = ((struct lpc17_40_list_s *)ret)->flink;
    }
...
static void lpc17_40_freeio(uint8_t *buffer)
{
  struct lpc17_40_list_s *iofree;
  irqstate_t flags;

  /* Could be called from the interrupt level */

  flags         = enter_critical_section();
  iofree        = (struct lpc17_40_list_s *)buffer;
...
static struct lpc17_40_xfrinfo_s *lpc17_40_alloc_xfrinfo(void)
{
  struct lpc17_40_xfrinfo_s *ret;
  irqstate_t flags;

  /* lpc17_40_free_xfrinfo() may be called from the interrupt level */

  flags = enter_critical_section();
  ret = (struct lpc17_40_xfrinfo_s *)g_xfrfree;
  if (ret)
    {
      g_xfrfree = ((struct lpc17_40_list_s *)ret)->flink;
    }
...
static void lpc17_40_free_xfrinfo(struct lpc17_40_xfrinfo_s *xfrinfo)
{
  struct lpc17_40_list_s *node;
  irqstate_t flags;

  /* Could be called from the interrupt level */

  flags        = enter_critical_section();
  node         = (struct lpc17_40_list_s *)xfrinfo;
                                     ...
                                     struct lpc17_40_ed_s *ed)
{
  ...
  ed->hw.nexted = lpc17_40_getreg(LPC17_40_USBHOST_CTRLHEADED);
  lpc17_40_putreg((uint32_t)ed, LPC17_40_USBHOST_CTRLHEADED);

  /* Re-enable control list processing. */

  lpc17_40_putreg(0, LPC17_40_USBHOST_CTRLED);
                                     ...
                                     struct lpc17_40_ed_s *ed)
{
  ...
  lpc17_40_putreg(0, LPC17_40_USBHOST_CTRLED);
  if (lpc17_40_getreg(LPC17_40_USBHOST_CTRLHEADED) != 0)
    {
                                     ...
                                     struct lpc17_40_ed_s *ed)
{
  ...
  ed->hw.nexted = lpc17_40_getreg(LPC17_40_USBHOST_BULKHEADED);
  lpc17_40_putreg((uint32_t)ed, LPC17_40_USBHOST_BULKHEADED);

  /* Re-enable bulk list processing. */

  lpc17_40_putreg(0, LPC17_40_USBHOST_BULKED);
                                     ...
                                     struct lpc17_40_ed_s *ed)
{
  ...
  lpc17_40_putreg(0, LPC17_40_USBHOST_BULKED);
  if (lpc17_40_getreg(LPC17_40_USBHOST_BULKHEADED) != 0)
    {
                                    ...
                                    struct lpc17_40_ed_s *ed)
{
  ...
  interval     = lpc17_40_getinterval(epdesc->interval);
                           ...
                           uint8_t *buffer, size_t buflen)
{
  ...
  xfrinfo = lpc17_40_alloc_xfrinfo();
  if (xfrinfo == NULL)
    {
  ...
  memset(xfrinfo, 0, sizeof(struct lpc17_40_xfrinfo_s));
  ...
  if (ret == OK)
    {
      ...
      lpc17_40_putreg(regval, LPC17_40_USBHOST_CMDST);
              ...
              if (xfrinfo)
                {
                  ...
                  next = (struct lpc17_40_gtd_s *)td->hw.nexttd;
                  lpc17_40_tdfree(td);
...
#ifdef CONFIG_DEBUG_USB
      if ((pending & LPC17_40_DEBUG_INTS) != 0)
        {
...
#endif
    {
      ret = lpc17_40_rh_enumerate(conn, hport);
      if (ret < 0)
        {
          return ret;
        }
                            ...
                            usbhost_ep_t *ep)
{
  ...
  ed = (struct lpc17_40_ed_s *)g_edfree;
  if (ed)
    {
      /* Remove the ED from the freelist */

      g_edfree = ((struct lpc17_40_list_s *)ed)->flink;

      /* Configure the endpoint descriptor. */

      memset((void *)ed, 0, sizeof(struct lpc17_40_ed_s));
      ...
      switch (ed->xfrtype)
        ...
        case USB_EP_ATTR_XFER_CONTROL:
          ret = lpc17_40_addctrled(priv, ed);
          break;

        case USB_EP_ATTR_XFER_BULK:
          ret = lpc17_40_addbulked(priv, ed);
          break;

        case USB_EP_ATTR_XFER_INT:
          ret = lpc17_40_addinted(priv, epdesc, ed);
          break;

        case USB_EP_ATTR_XFER_ISOC:
          ret = lpc17_40_addisoced(priv, epdesc, ed);
          break;
...
static int lpc17_40_epfree(struct usbhost_driver_s *drvr, usbhost_ep_t ep)
{
  ...
  switch (ed->xfrtype)
    {
    case USB_EP_ATTR_XFER_CONTROL:
      ret = lpc17_40_remctrled(priv, ed);
      break;

    case USB_EP_ATTR_XFER_BULK:
      ret = lpc17_40_rembulked(priv, ed);
      break;

    case USB_EP_ATTR_XFER_INT:
      ret = lpc17_40_reminted(priv, ed);
      break;

    case USB_EP_ATTR_XFER_ISOC:
      ret = lpc17_40_remisoced(priv, ed);
      break;
...
#if LPC17_40_IOBUFFERS > 0
  if (buflen <= CONFIG_LPC17_40_USBHOST_IOBUFSIZE)
    {
      uint8_t *alloc = lpc17_40_allocio();
      if (alloc)
        {
          *buffer = alloc;
          return OK;
...
/****************************************************************************
 * Name: lpc17_40_ctrlin and lpc17_40_ctrlout
 *
 * Description:
 * Description:
      ...
      if (buflen > CONFIG_LPC17_40_USBHOST_IOBUFSIZE)
        {
          uinfo("buflen (%d) > IO buffer size (%d)\n",
                 buflen, CONFIG_LPC17_40_USBHOST_IOBUFSIZE);
          return -ENOMEM;
        }

      /* Allocate an IO buffer in AHB SRAM */

      newbuffer = lpc17_40_allocio();
      if (!newbuffer)
        {
                                 ...
                                 size_t buflen)
{
  ...
  ret = lpc17_40_dma_alloc(priv, ed, buffer, buflen, &alloc);
  if (ret < 0)
    {
  ...
  ret = lpc17_40_transfer_common(priv, ed, buffer, buflen);
  if (ret < 0)
    {
      uerr("ERROR: lpc17_40_transfer_common failed: %d\n", ret);
      nbytes = (ssize_t)ret;
...
errout_with_buffers:
  ...
  lpc17_40_dma_free(priv, ed, userbuffer, buflen, alloc);
#endif
...
/****************************************************************************
 * Name: lpc17_40_asynch_completion
 *
 * Description:
                           ...
                           usbhost_asynch_t callback, void *arg)
{
  ...
  ret = lpc17_40_dma_alloc(priv, ed, buffer, buflen, &xfrinfo->alloc);
  if (ret < 0)
    {
      uerr("ERROR: lpc17_40_dma_alloc failed: %d\n", ret);
      goto errout_with_lock;
...
errout_with_asynch:
  ...
  lpc17_40_dma_free(priv, ed, buffer, buflen, xfrinfo->alloc);
#endif
  ...
  if (xfrinfo)
    {
      ...
      if (xfrinfo->wdhwait)
#endif
        {
          ...
          if (ed->xfrtype == USB_EP_ATTR_XFER_BULK)
            {
              ...
              lpc17_40_putreg(0, LPC17_40_USBHOST_BULKED);
              lpc17_40_putreg(ctrl, LPC17_40_USBHOST_CTRL);
            }
          else
            {
              /* Remove the TDs attached to the ED, keeping the Ed in the
          ...
          while (td != (struct lpc17_40_gtd_s *)LPC17_40_TDTAIL_ADDR)
            {
              next = (struct lpc17_40_gtd_s *)td->hw.nexttd;
              lpc17_40_tdfree(td);
              td = next;
...
static inline void lpc17_40_ep0init(struct lpc17_40_usbhost_s *priv)
{
  ...
  memset(EDCTRL, 0, sizeof(struct lpc17_40_ed_s));
  ...
  lpc17_40_putreg(0, LPC17_40_USBHOST_CTRLHEADED);

  /* Then add EP0 to the empty Control List */

  lpc17_40_addctrled(priv, EDCTRL);
}
...
/****************************************************************************
 * Name: lpc17_40_usbhost_initialize
 *
 * Description:
...
struct usbhost_connection_s *lpc17_40_usbhost_initialize(int controller)
{
  struct lpc17_40_usbhost_s *priv = &g_usbhost;
  struct usbhost_driver_s *drvr;
  struct usbhost_hubport_s *hport;
  ...
  DEBUGASSERT(sizeof(struct lpc17_40_ed_s)  <= LPC17_40_ED_SIZE);
  ...
  drvr                 = &priv->drvr;
  drvr->ep0configure   = lpc17_40_ep0configure;
  drvr->epalloc        = lpc17_40_epalloc;
  drvr->epfree         = lpc17_40_epfree;
  drvr->alloc          = lpc17_40_alloc;
  drvr->free           = lpc17_40_free;
  drvr->ioalloc        = lpc17_40_ioalloc;
  drvr->iofree         = lpc17_40_iofree;
  drvr->ctrlin         = lpc17_40_ctrlin;
  drvr->ctrlout        = lpc17_40_ctrlout;
  drvr->transfer       = lpc17_40_transfer;
#ifdef CONFIG_USBHOST_ASYNCH
  drvr->asynch         = lpc17_40_asynch;
#endif
  drvr->cancel         = lpc17_40_cancel;
#ifdef CONFIG_USBHOST_HUB
  drvr->connect        = lpc17_40_connect;
#endif
  drvr->disconnect     = lpc17_40_disconnect;
  ...
  do
    {
      regval = lpc17_40_getreg(LPC17_40_USBOTG_CLKST);
    }
  ...
  lpc17_40_putreg(1, LPC17_40_USBOTG_STCTRL);
  ...
  lpc17_40_configgpio(GPIO_USB_PPWR);    /* Port Power enable signal for USB port */
  lpc17_40_configgpio(GPIO_USB_PWRD);    /* Power Status for USB port (host power switch) */
  lpc17_40_configgpio(GPIO_USB_OVRCR);   /* USB port Over-Current status */
  usbhost_dumpgpio();
  ...
  buffer = (uint8_t *)LPC17_40_EDFREE_BASE;
  for (i = 0; i < CONFIG_LP17_USBHOST_NEDS; i++)
    {
      /* Put the ED in a free list */

      lpc17_40_edfree((struct lpc17_40_ed_s *)buffer);
      buffer += LPC17_40_ED_SIZE;
    }

  /* Initialize user-configurable TDs */

  buffer = (uint8_t *)LPC17_40_TDFREE_BASE;
  for (i = 0; i < CONFIG_LP17_USBHOST_NTDS; i++)
    {
      /* Put the TD in a free list */

      lpc17_40_tdfree((struct lpc17_40_gtd_s *)buffer);
      buffer += LPC17_40_TD_SIZE;
    }

  /* Initialize user-configurable request/descriptor transfer buffers */

  buffer = (uint8_t *)LPC17_40_TBFREE_BASE;
  for (i = 0; i < CONFIG_LPC17_40_USBHOST_TDBUFFERS; i++)
    {
      /* Put the TD buffer in a free list */

      lpc17_40_tbfree(buffer);
      buffer += CONFIG_LPC17_40_USBHOST_TDBUFSIZE;
    }

#if LPC17_40_IOBUFFERS > 0
  /* Initialize user-configurable IO buffers */

  buffer = (uint8_t *)LPC17_40_IOFREE_BASE;
  for (i = 0; i < LPC17_40_IOBUFFERS; i++)
    {
      /* Put the IO buffer in a free list */

      lpc17_40_freeio(buffer);
      buffer += CONFIG_LPC17_40_USBHOST_IOBUFSIZE;
    }
#endif
  ...
  for (i = 0, xfrinfo = g_xfrbuffers;
       i < CONFIG_LPC17_40_USBHOST_NPREALLOC;
       i++, xfrinfo++)
    {
  ...
  lpc17_40_putreg(OHCI_CMDST_HCR, LPC17_40_USBHOST_CMDST);
  ...
  lpc17_40_putreg(DEFAULT_PERSTART, LPC17_40_USBHOST_PERSTART);
  ...
  lpc17_40_putreg(OHCI_RHSTATUS_SGP, LPC17_40_USBHOST_RHSTATUS);

  /* Set HCCA base address */

  lpc17_40_putreg((uint32_t)HCCA, LPC17_40_USBHOST_HCCA);

  /* Set up the root hub port EP0 */

  lpc17_40_ep0init(priv);

  /* Clear pending interrupts */

  regval = lpc17_40_getreg(LPC17_40_USBHOST_INTST);
  lpc17_40_putreg(regval, LPC17_40_USBHOST_INTST);
  ...
  regval          = lpc17_40_getreg(LPC17_40_USBHOST_RHPORTST1);
  priv->connected = ((regval & OHCI_RHPORTST_CCS) != 0);

  /* Enable interrupts at the interrupt controller */

  up_enable_irq(LPC17_40_IRQ_USB); /* enable USB interrupt */
