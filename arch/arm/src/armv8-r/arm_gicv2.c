/****************************************************************************
 * arch/arm/src/armv8-r/arm_gicv2.c
 *
...
static void arm_gic0_init(void)
{
  unsigned int nlines = arm_gic_nlines();
  unsigned int irq;
...
#ifdef CONFIG_ARMV8R_GIC_EOIMODE
#  if defined(CONFIG_ARCH_TRUSTZONE_SECURE)
...
{
#if CONFIG_ARMV8R_GICV2_LEGACY_IRQ0 >= 0
  uint8_t slot;
  uint8_t tmp;
  ...
  slot = PCI_SLOT(devfn);
  tmp = (pin - 1 + slot) % 4;
  return CONFIG_ARMV8R_GICV2_LEGACY_IRQ0 + tmp;
#else
  return -ENOTSUP;
#endif
}
