/****************************************************************************
 * arch/arm/src/armv7-r/arm_addrenv.c
 *
...
int up_addrenv_select(const arch_addrenv_t *addrenv)
{
  ...
  if (addrenv->heap && (addrenv->data == 0 ||
    ...
    mpu_modify_region(g_addrenv_heap_region, addrenv->heap,
                      addrenv->heapsize,
                      MPU_RACR_S | MPU_RACR_C | MPU_RACR_AP_RWRW);
    }
...
int up_addrenv_ustackswitch(struct tcb_s *tcb)
{
  ...
  flags = MPU_RACR_S | MPU_RACR_C | MPU_RACR_AP_RWRW;

  if (g_addrenv_stack_region == 0)
    {
      g_addrenv_stack_region = mpu_allocregion();
    }

  mpu_modify_region(g_addrenv_stack_region,
                    (uintptr_t)tcb->stack_alloc_ptr,
                    tcb->adj_stack_size + tls->tl_size, flags);
  return OK;
}
...
int up_addrenv_kstackswitch(struct tcb_s *tcb)
{
  ...
  flags = MPU_RACR_S | MPU_RACR_C | MPU_RACR_AP_RWRO;
