...
static irqstate_t pm_runtime_lock(FAR struct pm_runtime_s *rpm)
{
                     ...
                     FAR struct pm_runtime_ops_s *ops)
{
  ...
  nxrmutex_init(&rpm->mutex);
  rspin_lock_init(&rpm->lock);
  rpm->use_count = 0;
  rpm->suspend_delay = 0;
  rpm->state = state;
  rpm->ops = ops;
