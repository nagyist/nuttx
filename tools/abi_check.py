...
def resolve_type(dwarfinfo, die):
    ...
    def get_name(die, prefix="", suffix="", defualt="void", size=0):
        name = die.attributes.get("DW_AT_name")
        return (
            f"{prefix}{name.value.decode('utf-8')}{suffix}" if name else defualt,
            size,
        )
    ...
    if die.tag == "DW_TAG_typedef":
        ...
        return get_name(die, defualt="unknown", size=size)

    if die.tag == "DW_TAG_structure_type":
        return get_name(
            die, prefix="struct ", defualt="anonymous struct", size=type_size
        )

    if die.tag == "DW_TAG_union_type":
        return get_name(die, prefix="union ", defualt="anonymous union", size=type_size)

    if die.tag == "DW_TAG_enumeration_type":
        return get_name(die, prefix="enum ", defualt="anonymous enum", size=type_size)
...
and output the result to lib_<json> file
    ...
    if args.struct_check and args.elf is None:
        print("Erorr: elf must be provided")
        exit(1)
