...
def dynamic_foreach_macro_write(file, board_list):
    ...
    for bid in board_list:
        ...
        foreach_macro += "      { \\\n"
        foreach_macro += f"        fn_foreach(DT_CAT_EVAL(B{bid}_, node_id), fn); \\\n"
        foreach_macro += "      } \\\n"
    foreach_macro += "    else \\\n"
    foreach_macro += "      { \\\n"
    ...
    for bid in board_list:
        ...
        foreach_vargs_macro += "      { \\\n"
        foreach_vargs_macro += (
            f"        fn_foreach(DT_CAT_EVAL(B{bid}_, node_id), fn, __VA_ARGS__); \\\n"
        )
        foreach_vargs_macro += "      } \\\n"
    ...
    for bid in board_list:
        ...
        foreach_status_okay_macro += "      { \\\n"
        foreach_status_okay_macro += (
            f"        COND_CODE_1(DT_CAT3_EVAL(B{bid}_, DT_COMPAT_HAS_OKAY_, compat), \\\n"
            f"        (DT_CAT3_EVAL(B{bid}_, DT_FOREACH_OKAY_, compat)(fn)), \\\n"
            f"        ()) \\\n"
        )
        foreach_status_okay_macro += "      } \\\n"
    ...
    foreach_status_okay_macro += "      { \\\n"
    foreach_status_okay_macro += (
        "        COND_CODE_1(DT_CAT_EVAL(DT_COMPAT_HAS_OKAY_, compat), \\\n"
    )
    foreach_status_okay_macro += (
        "        (DT_CAT_EVAL(DT_FOREACH_OKAY_, compat)(fn)), \\\n"
    )
    foreach_status_okay_macro += "        ()) \\\n"
    foreach_status_okay_macro += "      } \\\n"
    ...
    for bid in board_list:
        ...
        foreach_status_okay_vagrs_macro += (
            f"        COND_CODE_1(DT_CAT3_EVAL(B{bid}_, DT_COMPAT_HAS_OKAY_, compat), \\\n"
            f"        (DT_CAT3_EVAL(B{bid}_, DT_FOREACH_OKAY_VARGS_, compat)(fn, __VA_ARGS__)), \\\n"
            f"        ()) \\\n"
        )
        foreach_status_okay_vagrs_macro += "      } \\\n"
    ...
    foreach_status_okay_vagrs_macro += (
        "        (DT_CAT_EVAL(DT_FOREACH_OKAY_VARGS_, compat)(fn, __VA_ARGS__)), \\\n"
    )
    foreach_status_okay_vagrs_macro += "        ()) \\\n"
    foreach_status_okay_vagrs_macro += "      } \\\n"
