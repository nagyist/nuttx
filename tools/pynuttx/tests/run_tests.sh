#! /bin/bash
############################################################################
# tools/pynuttx/tests/run_tests.sh
#
...
PACKET_PATH=$(python -m site | grep 'site-packages' | head -n 1 | sed 's/^[ \t]*//;s/[ \t]*$//')
...
-ex="${EXTRA_CMD}" \
-ex="c" \
-ex="python import sys; sys.path.append(${PACKET_PATH}); print(sys.version); print(sys.executable); print(sys.path)" \
