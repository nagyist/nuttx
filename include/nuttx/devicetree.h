...
#ifndef __INCLUDE_NUTTX_DEVICETREE_H
#define __INCLUDE_NUTTX_DEVICETREE_H

/****************************************************************************
 * Included Files
 ****************************************************************************/

#include <nuttx/devicetree_generated.h>

#if !defined(_LINKER) && !defined(_ASMLANGUAGE)
#include <stdint.h>
#endif

#include <nuttx/devicetree/util.h>
 ...
 * _FOREACH_PROP_ELEM_VARGS: foreach functions with variable number of
 *                           arguments
 ...
 * _IDX_<i>_STRING_UPPER_TOKEN: string array element value as a uppercased
 *                              token
 * _IDX <i>_STRING_UNQUOTED: string array element value as a sequence of
 *                           tokens, with no quotes
 ...
 * _STRING_UNQUOTED: string property's value as a sequence of tokens,
 *                   with no quotes
 */
 ...
 * devicetree data may or may not be available. It is a preprocessor
 * identifier that does not match any valid devicetree node identifier.
 */
...
/**
 * @brief Get the node's full name, including the unit-address, as an
 *        unquoted sequence of tokens
 *
 ...
 *
 * @param node_id node identifier
 * @return the node's full name with unit-address as a token, i.e. without
 ...
 * See include/nuttx/devicetree/ordinals.h.
 * @endinternal
 *
 ...
 *
 * @param node_id node identifier
 * @return an array initializer for an array of the node's node labels as
 *         strings
 */
 ...
 *
 * It is an error to use this macro with the `reg` or `interrupts`
 * properties.
 *
 * @param node_id node identifier
 * @param prop a lowercase-and-underscores property with a logical length
...
/**
 * @brief Like DT_ENUM_IDX_BY_IDX(), but with a fallback to a default enum
 *        index
 *
...
/**
 * @brief Equivalent to DT_ENUM_IDX_BY_IDX_OR(node_id, prop, 0,
 *        default_idx_value).
 * @param node_id node identifier
 * @param prop lowercase-and-underscores property name
 * @param default_idx_value a fallback index value to expand to
...
/**
 * @brief Like DT_STRING_UPPER_TOKEN(), but with a fallback to
 *        @p default_value
 *
 ...
 * non-alphanumeric characters to underscores. That can be useful, for
 * example, when programmatically using the value to form a C variable
 * or code.
 *
 ...
 * converts non-alphanumeric characters to underscores. That can be useful,
 * for example, when programmatically using the value to form a C variable
 * or code.
 *
 * DT_STRING_UPPER_TOKEN_BY_IDX() can only be used for properties with
 * string-array type.
 *
 * It is an error to use DT_STRING_UPPER_TOKEN_BY_IDX() in other
 * circumstances.
 *
 * Example devicetree fragment:
 *
 ...
 *
 *     // "ADC_1"
 *     DT_PROP(DT_PHANDLE_BY_NAME(NODE, io_channels, sensor), foobar)
 *     // "ADC_2"
 *     DT_PROP(DT_PHANDLE_BY_NAME(NODE, io_channels, bandgap), foobar)
 * @endcode
 *
 ...
 *
 * If this returns 1, then DT_RANGES_CHILD_BUS_FLAGS_BY_IDX(node_id, idx)
 * is valid. If it returns 0, it is an error to use this macro with index
 * @p idx. This macro only returns 1 for PCIe buses (i.e. nodes whose
 * bindings specify they are "pcie" bus nodes.)
 *
 * Example devicetree fragment:
 *
 * @code{.dts}
 ...
 *
 * When the node is a PCIe bus, the Child Bus Address has an extra cell used
 * to store some flags, thus this cell is extracted from the Child Bus
 *  Address as Child Bus Flags field.
 *
 ...
 *
 * When the node is a PCIe bus, the Child Bus Address has an extra cell used
 * to store some flags, thus this cell is removed from the Child Bus Address.
 *
 * Example devicetree fragments:
 *
 * @code{.dts}
 ...
 * @code{.c}
 *     // 0
 *     DT_RANGES_CHILD_BUS_ADDRESS_BY_IDX(DT_NODELABEL(pcie0), 0)
 *     // 0x10000000
 *     DT_RANGES_CHILD_BUS_ADDRESS_BY_IDX(DT_NODELABEL(pcie0), 1)
 *     // 0x8000000000
 *     DT_RANGES_CHILD_BUS_ADDRESS_BY_IDX(DT_NODELABEL(pcie0), 2)
 *     // 0
 *     DT_RANGES_CHILD_BUS_ADDRESS_BY_IDX(DT_NODELABEL(other), 0)
 *     // 0x10000000
 *     DT_RANGES_CHILD_BUS_ADDRESS_BY_IDX(DT_NODELABEL(other), 1)
 * @endcode
 *
 * @param node_id node identifier
 * @param idx logical index into the ranges array
 ...
 * @code{.c}
 *     // 0x3eff0000
 *     DT_RANGES_PARENT_BUS_ADDRESS_BY_IDX(DT_NODELABEL(pcie0), 0)
 *     // 0x10000000
 *     DT_RANGES_PARENT_BUS_ADDRESS_BY_IDX(DT_NODELABEL(pcie0), 1)
 *     // 0x8000000000
 *     DT_RANGES_PARENT_BUS_ADDRESS_BY_IDX(DT_NODELABEL(pcie0), 2)
 *     // 0x3eff0000
 *     DT_RANGES_PARENT_BUS_ADDRESS_BY_IDX(DT_NODELABEL(other), 0)
 *     // 0x10000000
 *     DT_RANGES_PARENT_BUS_ADDRESS_BY_IDX(DT_NODELABEL(other), 1)
 * @endcode
 *
 * @param node_id node identifier
 * @param idx logical index into the ranges array
 ...
 *
 * The macro @p fn must take two parameters, @p node_id which will be the
 * node identifier of the node with the ranges property and @p idx the
 * index of the ranges block.
 *
 * Example devicetree fragment:
 *
 * @code{.dts}
 ...
 *
 * @code{.c}
 *     #define RANGE_LENGTH(node_id, idx) \
 ...
 *  nuttx  NuttX-specific binding
 *
 * Example devicetree fragment:
 *
 * @code{.dts}
 *  n1: node-1 {
 *      compatible = "vnd,model1", "gpio", "nuttx,model2";
 *  };
 * @endcode
 *
 * Example usage:
 *
 * @code{.c}
 *  // "A stand-in for a real vendor"
 *  DT_NODE_VENDOR_BY_IDX(DT_NODELABEL(n1), 0)
 *  // "NuttX-specific binding"
 *  DT_NODE_VENDOR_BY_IDX(DT_NODELABEL(n1), 2)
 * @endcode
 *
 ...
 *
 * If this returns 1, then DT_NODE_VENDOR_BY_IDX(node_id, idx) is valid.
 * If it returns 0, it is an error to use
 * DT_NODE_VENDOR_BY_IDX(node_id, idx) with index @p idx.
 *
 * @param node_id node identifier
 ...
 *
 * The model is a string extracted from the compatible after the
 * vendor prefix.
 *
 * Example vendor-prefixes.txt:
 *
 ...
 * type suffix. This can be used to index other devicetree properties,
 * use the non _RAW macros for assigning values in actual code.
 *
 * @param node_id node identifier
 ...
 *
 * @param node_id node identifier
 * @return Number of interrupt specifiers in the node's "interrupts"
 *         property.
 */
...
/**
 * @brief Does an interrupts property have a named cell specifier at an
 *        index?
...
/**
 * @brief Does an interrupts property have a named specifier value at an
 *        index?
...
/* DT helper macro to encode a node's IRQN to level 1 according
 * to the multi-level scheme
 */
#define DT_IRQN_L1_INTERNAL(node_id, idx) DT_IRQ_BY_IDX(node_id, idx, irq)

/* DT helper macro to encode a node's IRQN to level 2 according
 * to the multi-level scheme
 */
...
/* DT helper macro to encode a node's IRQN to level 3 according
 * to the multi-level scheme
 */
...
#define DT_IRQN_LVL_INTERNAL(node_id, idx, level) \
    DT_CAT3(DT_IRQN_L, level, _INTERNAL)(node_id, idx)

/**
 * DT helper macro to encode a node's interrupt number according
 * to the multi-level scheme
 */
...
/**
 * @brief Get the node's interrupt number at index
 * If @kconfig{CONFIG_MULTI_LEVEL_INTERRUPTS} is enabled, the interrupt
 * number at index will be multi-level encoded
 * @param node_id node identifier
 * @param idx logical index into the interrupt specifier array
...
/**
 * @brief Invokes @p fn for every status `okay` node in the tree with
 *        multiple arguments.
 *
 * The macro @p fn takes multiple arguments. The first should be the node
 * identifier for the node. The remaining are passed-in by the caller.
 ...
 * (as usual, a missing status property is treated as status `okay`). The
 * order that nodes are visited in is not specified.
 *
 * @param fn macro to invoke
 * @param ... variable number of arguments to pass to @p fn
 */
...
/**
 * @brief Invokes @p fn for each child of @p node_id with separator and
 *        multiple arguments.
 *
 * The macro @p fn takes multiple arguments. The first should be the node
 * identifier for the child node. The remaining are passed-in by the caller.
 ...
 *     int bar = 1;
 *
 *     #define VAR_PLUS(nodelabel, to_add) int nodelabel ## _added = \
 *             nodelabel + to_add;
 *
 ...
 *
 * @param inst instance number
 * @return an array initializer for an array of the instance's node labels
 *         as strings.
 */
 ...
 *
 * @param inst instance number
 * @return the number of node labels that the node with that instance number
 *         has.
 */
...
/**
 * @brief Call @p fn on all child nodes of DT_DRV_INST(inst) with status
 * `okay`.
 *
 * The macro @p fn should take one argument, which is the node
 * identifier for the child node.
...
/**
 * @brief Call @p fn on all child nodes of DT_DRV_INST(inst) with status
 * `okay` and with separator and multiple arguments.
 *
 * The macro @p fn takes multiple arguments. The first should be the node
 * identifier for the child node. The remaining are passed-in by the caller.
 *
...
/**
 * @brief Get a `DT_DRV_COMPAT` property array value's index into its
 *        enumeration values
 * @param inst instance number
 * @param prop lowercase-and-underscores property name
 * @param idx the index to get
...
/**
 * @brief Like DT_INST_ENUM_IDX_BY_IDX(), but with a fallback to a default
 *        enum index
 * @param inst instance number
 * @param prop lowercase-and-underscores property name
 * @param idx the index to get
...
/**
 * @brief Like DT_INST_ENUM_IDX(), but with a fallback to a default enum
 *        index
 * @param inst instance number
 * @param prop lowercase-and-underscores property name
 * @param default_idx_value a fallback index value to expand to
...
/**
 * @brief Does a `DT_DRV_COMPAT` enumeration property have a given value by
 *        index?
 * @param inst instance number
 * @param prop lowercase-and-underscores property name
 * @param idx the index to get
...
/**
 * @brief Get an element out of string-array property as an unquoted sequence
 *        of tokens.
 * @param inst instance number
 * @param prop lowercase-and-underscores property name
 * @param idx the index to get
 * @return the value of @p prop at index @p idx as a sequence of tokens, with
 *         no quotes
 */
...
/**
 * @brief Get a `DT_DRV_COMPAT` instance's property value from a phandle's
 *        node
 * @param inst instance number
...
/**
 * @brief Get a `DT_DRV_COMPAT` instance's phandle-array specifier value at
 *        an index
 * @param inst instance number
 * @param pha lowercase-and-underscores property with type `phandle-array`
...
/**
 * @brief is @p idx a valid register block index on a `DT_DRV_COMPAT`
 *        instance?
 * @param inst instance number
 * @param idx index to check
 * @return 1 if @p idx is a valid register block index,
...
/**
 * @brief is @p name a valid register block name on a `DT_DRV_COMPAT`
 *        instance?
 * @param inst instance number
 * @param name name to check
 * @return 1 if @p name is a valid register block name,
...
/**
 * @brief Get a `DT_DRV_COMPAT` instance's idx-th register block's raw
 *        address
 * @param inst instance number
 * @param idx index of the register whose address to return
...
/**
 * @brief Like DT_INST_REG_ADDR_BY_NAME(), but with a fallback to
 *        @p default_value
 * @param inst instance number
 * @param name lowercase-and-underscores register specifier name
 * @param default_value a fallback value to expand to
...
/**
 * @brief Like DT_INST_REG_SIZE_BY_NAME(), but with a fallback to
 *        @p default_value
 * @param inst instance number
 * @param name lowercase-and-underscores register specifier name
 * @param default_value a fallback value to expand to
 ...
 * @param inst instance number
 * @param bus a binding's bus type as a C token, lowercased and without
 *            quotes
...
/**
 * @brief Like DT_INST_STRING_TOKEN(), but with a fallback to
 *        @p default_value
 * @param inst instance number
 * @param name lowercase-and-underscores property name
 * @param default_value a fallback value to expand to
 * @return if @p prop exists, its value as a token, i.e. without any quotes
 *         and with special characters converted to underscores. Otherwise
 *         @p default_value
 */
 ...
 *
 * This differs from @ref DT_ANY_INST_HAS_PROP_STATUS_OKAY because even when
 * not present on a node, the boolean property is generated with a value of
 * 0 and therefore exists.
 *
 * @param prop lowercase-and-underscores property name
 *
 * Example devicetree overlay:
...
/**
 * @brief is index valid for interrupt property on a `DT_DRV_COMPAT`
 *        instance?
 * @param inst instance number
 * @param idx logical index into the interrupt specifier array
...
/**
 * @brief Does a `DT_DRV_COMPAT` instance have an interrupt named cell
 *        specifier?
 * @param inst instance number
 * @param idx index to check
 * @param cell named cell value whose existence to check
 ...
 * @param inst instance number
 * @param cell named cell value whose existence to check
 * @return 1 if the named @p cell exists in the interrupt specifier at
 *         index 0, 0 otherwise.
 */
 ...
 * DT_INST_FOREACH_STATUS_OKAY_VARG to generate comma separated list of 1,
 * where each 1 on the list represents instance that has a property
 * @p prop of value 1; the list may be empty, and the upper bound on number
 * of list elements is number of device instances.
 *
 * @param prop property to check
 *
 * @return Evaluates to list of 1s (e.g: 1,1,1,) or nothing.
 ...
 */
#define DT_CAT_EVAL(a1, a2) DT_CAT(a1, a2)
/** @brief Concatenation helper with evaluation, 3 arguments */
#define DT_CAT3_EVAL(a1, a2, a3) DT_CAT3(a1, a2, a3)
/** @brief Concatenation helper with evaluation, 4 arguments */
#define DT_CAT4_EVAL(a1, a2, a3, a4) DT_CAT4(a1, a2, a3, a4)
/** @brief Concatenation helper with evaluation, 5 arguments */
#define DT_CAT5_EVAL(a1, a2, a3, a4, a5) DT_CAT5(a1, a2, a3, a4, a5)
/** @brief Concatenation helper with evaluation, 6 arguments */
#define DT_CAT6_EVAL(a1, a2, a3, a4, a5, a6) \
    DT_CAT6(a1, a2, a3, a4, a5, a6)
/** @brief Concatenation helper with evaluation, 7 arguments */
#define DT_CAT7_EVAL(a1, a2, a3, a4, a5, a6, a7) \
    DT_CAT7(a1, a2, a3, a4, a5, a6, a7)
/** @brief Concatenation helper with evaluation, 8 arguments */
#define DT_CAT8_EVAL(a1, a2, a3, a4, a5, a6, a7, a8) \
    DT_CAT8(a1, a2, a3, a4, a5, a6, a7, a8)
...
/**
 * @def DT_U32_C
 * @brief Macro to add 32bit unsigned postfix to the devicetree address
 *        constants
 */
#if defined(_LINKER) || defined(_ASMLANGUAGE)
...
#include <nuttx/devicetree/ordinals.h>

#endif /* __INCLUDE_NUTTX_DEVICETREE_H */
