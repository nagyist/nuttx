...
#ifndef __INCLUDE_NUTTX_DEVICETREE_UTIL_H
#define __INCLUDE_NUTTX_DEVICETREE_UTIL_H
...
/* needs to be outside ASSEMBLY so 'true' and 'false' can turn
 ...
 *
 * This macro can be seen as a slightly stricter version of @ref
 * PART_OF_ARRAY in that it also ensures that @p ptr is aligned to an
 * array-element boundary of @p array.
 *
 * In C, passing a pointer as @p array causes a compile error.
 *
 ...
 *
 * Concatenate provided tokens into a combined token during the preprocessor
 * pass. This can be used to, for ex., build an identifier out of multiple
 * parts, where one of those parts may be, for ex, a number, another macro,
 * or a macro argument.
 *
 ...
 * the NULL check within the macro. The compiler is still able to optimize
 * the NULL check out at a later stage.
 *
 * @param p Pointer to check
 * @return true if @p p is equal to ``NULL``, false otherwise
 */

static always_inline_function bool is_null_no_warn(void *p)
{
    return p == NULL;
}
 ...
 *
 * Copy `size` bytes of `src` into `dest`. This is guaranteed to be done byte
 * by byte.
 *
...
/**
 * @brief Sign extend an 8, 16 or 32 bit value using the index bit as sign
 * bit.
 *
 ...
 *
 * The resulting @p dst will always be NULL terminated if @p n is larger than
 * 0, and the @p dst string will always be properly UTF-8 truncated.
 *
 * @param dst The destination of the UTF-8 string.
 * @param src The source string
 * @param n   The size of the @p dst buffer. Maximum number of characters
 *            copied is @p n - 1. If 0 nothing will be done, and the @p dst
 ...
 *
 * @param x An unsigned integral value to compute logarithm of
 *          (positive only)
 *
...
static inline void
mem_xor_n(uint8_t *dst, const uint8_t *src1, const uint8_t *src2, size_t len)
{
    while (len--)
    {
        *dst++ = *src1++ ^ *src2++;
...
static inline void
mem_xor_32(uint8_t dst[4], const uint8_t src1[4], const uint8_t src2[4])
{
...
static inline void
mem_xor_128(uint8_t dst[16], const uint8_t src1[16], const uint8_t src2[16])
{
...
/* This file must be included at the end of the !__ASSEMBLY__ guard.
 * It depends on macros defined in this file above which cannot be forward
 * declared.
 */
#include <nuttx/devicetree/time_units.h>
...
#endif /* __INCLUDE_NUTTX_DEVICETREE_UTIL_H */
